...
def test_parse_processable_audio_mime_types_normalizes_entries():
    processable = parse_processable_audio_mime_types(
        " audio/mpeg , AUDIO/X-M4A; charset=binary ,, audio/flac , AUDIO/VND.WAVE; codecs=1 "
    )

    assert processable == frozenset({"audio/mpeg", "audio/x-m4a", "audio/flac", "audio/vnd.wave"})


def test_is_processable_audio_mime_type_handles_parameters_and_unknowns():
    processable = parse_processable_audio_mime_types("audio/mpeg,audio/wav,audio/webm,audio/vnd.wave")
    ...
    assert is_processable_audio_mime_type(" audio/webm ", processable) is True
    assert is_processable_audio_mime_type("audio/vnd.wave; codecs=1", processable) is True
