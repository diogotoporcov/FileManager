...
import com.diogotoporcov.filemanager.api.duplicate.persistence.AudioDuplicateCandidateRepository;
import com.diogotoporcov.filemanager.api.duplicate.persistence.ExactDuplicateCandidateRepository;
import com.diogotoporcov.filemanager.api.duplicate.persistence.ExactDuplicateGroupFileProjection;
import com.diogotoporcov.filemanager.api.duplicate.persistence.ExactDuplicateGroupKeyProjection;
...
public class DuplicateSearchService {
    ...
    private DuplicateMethodResultResponse searchExact(
        ...
        return fileFingerprintRepository.findByFileIdAndAlgorithm(sourceFile.getId(), EXACT_ALGORITHM)
                .map(sourceFingerprint -> {
                    List<com.diogotoporcov.filemanager.api.duplicate.persistence.ExactDuplicateCandidateProjection> candidates =
    ...
    private DuplicateMethodResultResponse searchImagePhash(
        ...
        return imageFingerprintRepository.findByFileId(sourceFile.getId())
                .map(sourceFingerprint -> {
                    List<com.diogotoporcov.filemanager.api.duplicate.persistence.PhashDuplicateCandidateProjection> candidates =
    ...
    private DuplicateMethodResultResponse searchAudioFingerprint(
        ...
        return audioFingerprintRepository.findByFileId(sourceFile.getId())
                .map(sourceFingerprint -> {
                    List<com.diogotoporcov.filemanager.api.duplicate.persistence.AudioDuplicateCandidateProjection> candidates =
    ...
    private SearchCursor embeddingNextCursor(
            List<com.diogotoporcov.filemanager.api.duplicate.persistence.EmbeddingDuplicateCandidateProjection> candidates,
            int pageSize,
            boolean forceHasMore) {
    ...
    private record AdaptiveEmbeddingCandidates(
            List<com.diogotoporcov.filemanager.api.duplicate.persistence.EmbeddingDuplicateCandidateProjection> matches,
            boolean forceHasMore) {
        java.util.stream.Stream<com.diogotoporcov.filemanager.api.duplicate.persistence.EmbeddingDuplicateCandidateProjection> stream() {
